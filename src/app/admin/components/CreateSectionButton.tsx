...
export function CreateSectionButton({
  ...
  onCreated?: (s: { id: string; name: string; slug: string }) => void
}) {
  ...
  async function create() {
    ...
    try {
      ...
      if (!res.ok) {
        toast({ type: "error", title: data.error || "Ошибка создания раздела" })
        return
      }
      toast({ type: "success", title: "Раздел создан" })
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="bg-white rounded-xl shadow w-full max-w-md p-4 space-y-3">
            <div className="flex items-center justify-between">
              <div className="text-lg font-medium">Новый раздел</div>
